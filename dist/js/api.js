...
export const api = {
  ...
  dialogue: input => {
    ...
    const conversationId = data.conversationId ? sanitize(data.conversationId, 120) : null;
    const clientMessageId = data.clientMessageId ? sanitize(data.clientMessageId, 120) : uuid();
    const headers = data.accessToken ? { authorization: `Bearer ${data.accessToken}` } : {};
    const body = { clientMessageId, message };
    if (conversationId) body.conversationId = conversationId;
    return request('/api/dialogue', { method: 'POST', headers, body: JSON.stringify(body) });
  },
