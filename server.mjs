...
async function serveFile(req,res,url){
  ...
  try{const s=await stat(filePath);if(s.isDirectory())filePath=path.join(filePath,'index.html');const ext=path.extname(filePath);const data=await readFile(filePath);const cachePolicy=ext==='.html'||ext==='.js'||ext==='.css'||path.basename(filePath)==='sw.js'?'public, max-age=0, must-revalidate':filePath.includes(`${path.sep}assets${path.sep}`)?'public, max-age=31536000, immutable':'public, max-age=300';return send(res,200,data,mime[ext]||'application/octet-stream',{'Cache-Control':cachePolicy})}catch{
